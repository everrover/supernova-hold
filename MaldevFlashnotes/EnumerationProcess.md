...
### Process enum - Essentially recon ops

A key step in injection process, identifying the target process.

APIs can hv ceveats, work around to find the target process.
- `EnumProcesses` only lists IDs. `OpenProcess` opens a handle, uses PID(`PROCESS_QUERY_INFORMATION | PROCESS_VM_READ` permissions). `EnumProcessModules` will be used to enumerate all the modules within the opened process. `GetModuleBaseName` gets process name.
- All 3 can be used to identify the process and for further exploitation.
...
const unsigned char EncodedPayload[] = {0x00};
```
